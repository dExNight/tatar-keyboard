...
**История переписана 2026-09-02** (решение оператора, `docs/AUTHORSHIP-PLAN.md`,
вариант B): из тел 226 коммитов удалены трейлеры `Co-Authored-By: Claude …`;
авторы и содержимое деревьев не изменены (проверено `git ls-tree`). Все хэши
коммитов в документах до `dfcfb118` ведут на прежнюю историю и больше не
разрешаются — принято, ссылки не обновлялись. Теги пересозданы (v1.9.4 →
`8875c274`). Трейлеры в будущем блокирует `.githooks/commit-msg`
