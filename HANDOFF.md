...
* **Открытый хвост движка (из ENGINE.md, до фичи):** первый NEXT_WORD-запрос
  сессии/свежего поля иногда отвечает пусто — гонка двухступенчатого attach
  биграммной таблицы (E5c); воспроизводится и на чистом 1.9.9, на релизе 1.9.10
  и на debug. Повторный набор слова в той же сессии показывает полосу
  полностью. На фичу влияет косвенно: первый «пустой» ответ не несёт и эмодзи.
* **Гейт пинов `release_check.sh` не покрывает emoji-ассеты** (только 4 ассета
  словарей/биграмм); emoji_suggest охраняется python/JVM-пинами внутри гейтов
  тестов. Свести — возможное улучшение DEV-плана.
