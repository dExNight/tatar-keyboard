...
1. Мусор, который оператор называл лично (`щрн`, `нб`, `фп`, `бш`), — в словаре или нет.
   Если хоть один в словаре, правило неверное, и об этом надо узнать здесь, а не от человека.
...
def main() -> None:
    ...
    for tag, silent, prefixes in (("rus", SILENT_RU, PREFIXES_RU),
        ...
        out[tag] = {
            "operator_garbage_in_dictionary": [w for w in OPERATOR_GARBAGE if w in af],
            "silent_words": {
