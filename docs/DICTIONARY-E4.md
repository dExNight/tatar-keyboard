...
Статус: фаза E4 выполнена целиком — **E4a-1**, **E4a-2**, **E4b**, **E4c** и **E4d**.
...
## E4c. Обучение на трёх чистых завершениях

Подфаза замыкает фичу: слова начинают попадать в личный словарь из набора, а не только из ручного
добавления. Один порог, один предикат, ни одного нового IPC и ни одного нового API движка.

### Событие «слово завершено» — из хуков, которые уже были

Чистый пробег: слово росло по одному куску (`w.startsWith(previous) && w.length > previous.length`)
и завершилось переходом trailing word в пустую строку. Грязным пробег помечают backspace и любое
сокращение, смена selection, внутренний жест курсора (LatinIME направляет его в тот же
`onSelectionChanged`), принятая подсказка и смена subtype.

**Решение, принятое строже контракта и названное прямо:** первое слово сессии не засчитывается
вообще. На старте поля мы ещё не наблюдали ни одной границы слова, поэтому trailing word может
оказаться текстом, который приложение подставило само, а пользователь лишь дописал. Пробег считается
чистым только начиная с границы, которую мы видели своими глазами. Цена — одно несосчитанное слово
на сессию; выигрыш — невозможность запомнить чужой предзаполненный текст. Запинено тестом
`theFirstWordOfASessionIsNotCountedAtAll`.

### Фильтр «слова нет в основном словаре»

Слово W засчитывается, только если для какого-то его СОБСТВЕННОГО префикса p (|p| < |W|), реально
запрошенного в этом же чистом пробеге, `applyResult` получил ПУСТОЙ список. Пустой результат для p
означает, что ни одно словарное слово, кроме самого p, не начинается с p, — значит и W в словаре
нет. Нового API у движка не появилось: используется результат, который и так приходил.

Запоминается ДЛИНА САМОГО КОРОТКОГО такого префикса, а не самого длинного. Это не микрооптимизация:
последний пустой результат пробега — обычно результат по всему слову целиком, и «самый длинный»
затирал бы им ровно то свидетельство, ради которого правило существует, после чего не запоминалось бы
никогда ничего. Ошибка была допущена при первой реализации и поймана тестом
`aWordSpelledOutCleanlyAndUnknownToTheDictionaryIsReportedOnce`.

Если наблюдения не было (coalescing схлопнул запросы, движок не был готов, поле неподходящее) —
слово не запоминается. Fail-closed в сторону МЕНЬШЕЙ записи.

### Порог и срок жизни pending

`LEARN_THRESHOLD` = 3 чистых завершения, счёт отдельный на subtype, константа кода, в настройки не
вынесена. Обоснование числа: 1 запоминает любую опечатку; 2 запоминает повторённую опечатку — одну и
ту же ошибку человек делает подряд; 3 требует, чтобы форма выжила три независимых завершения без
единого исправления.

Порог и срок жизни pending — два конца одного механизма, поэтому оба записаны здесь: `MAX_PENDING`
= 256 записей на subtype, `LIFETIME_SERIALS` = 500 — при каждом сбросе отбрасываются записи, чей
серийник отстал от текущего более чем на 500. Без этого правила слово, набранное однажды и никогда
больше, оставалось бы солёным хешем в файле до вытеснения 256-й записью или до «Стереть всё».

### Pending не хранит текст

Ключ — первые 8 байт `SHA-256(соль ‖ нормализованная NFC lowercase форма)`, значение — счётчик u16 и
серийник u32. Соль — 16 случайных байт в `salt.bin`, создаётся при первом использовании и
уничтожается вместе со всем остальным при «Стереть всё» (чтобы хеши следующей сессии нельзя было
сопоставить с хешами стёртой).

**Что это защищает и что нет, без эвфемизмов:** это защита от файла, извлечённого случайно —
бэкапом, которого не должно было быть, форензик-дампом, любопытным файловым менеджером. Это НЕ защита
от того, у кого есть и файл, и соль: имея оба, можно прохешировать словник кандидатов и сравнить.
Ключ считается по нормализованной форме, иначе «Гүзәл» и «гүзәл» вели бы отдельные счётчики и ни один
не дошёл бы до порога.

Частота записи: pending и счётчики использования держатся в памяти и пишутся на одной границе —
`SuggestionsController.onFinishInput()`, вызываемой из `LatinIME.onFinishInputViewInternal`, — и
только при изменении. Не на каждое нажатие и не на каждое завершённое слово. Тест
`theSessionEndFlushesExactlyOnce`.

### Предикат обучения — один, из пяти сомножителей

`LatinIME.mayLearnPersonalWords()`: `isTatarSuggestionsEligible()` И `PREF_PERSONAL_DICTIONARY` И
`UserManager.isUserUnlocked()` И «поле не является почтовым адресом». Пятый сомножитель — случай
`editorInfo == null` — закрыт первым: без editorInfo класс поля не `TYPE_CLASS_TEXT`, значит
`mShouldShowSuggestions` = false и eligibility не выполняется. Отдельной проверки для него нет
намеренно: она была бы вторым местом, где то же самое решается по-другому.

`IME_FLAG_NO_PERSONALIZED_LEARNING` тоже входит внутрь eligibility — унаследовано от D1e, и это
СТРОЖЕ контракта: контракт разрешает в таком поле показывать подсказки из уже накопленного словаря
(чтение — не сбор), а у нас там полоса не показывается вовсе. Строже — допустимо; записано, чтобы
следующий читатель не счёл это ошибкой.

Предикат разделяется путями записи: и завершение слова, и сброс на границе сессии спрашивают его
(два `if (!predicate.mayLearn()) return` в `PersonalLearning.sinkFor`, запинено тестом). Иначе
сессия, переставшая быть подходящей, всё равно положила бы накопленное на диск.

`UserManager.isUserUnlocked()` стоит в предикате не ради «лишней записи»: до разблокировки снапшот
пуст по построению, а запись выполняется только целиком, поэтому первое же сработавшее обучение
собрало бы файл из пустого снапшота плюс одного слова и атомарно заменило бы им реальный личный
словарь. Окно реально — directBootAware-приложение имеет поля ввода до разблокировки.

Почтовый адрес исключён ЛОКАЛЬНО, в предикате: `shouldSuppressSuggestions` не тронут, поэтому показ
подсказок в адресном поле ведёт себя ровно как раньше, а блокируется только запись. **Почему при этом
НЕ исключается `TYPE_TEXT_VARIATION_PERSON_NAME`:** имена — прямая цель фичи, ради них она и
делалась; исключить их значило бы выключить её главный сценарий. Запинено тестом, чтобы следующий
читатель не «закрыл заодно» и его.

### Решение по `minSdk` (контракт требует записать его здесь и в `PRIVACY.md`)

**`minSdk` остаётся 24; ограничение записано как известное.** Константа
`IME_FLAG_NO_PERSONALIZED_LEARNING` введена в API 26, поэтому на API 24–25 её не выставляет ни одно
приложение и механизм «не персонализироваться» на этих двух уровнях отсутствует. Поднятие `minSdk` до
26 сняло бы ограничение сразу и для «недавних» эмодзи, но отсекло бы часть самых дешёвых устройств —
ровно ту аудиторию, ради которой проект существует. Соответствующая строка внесена в `PRIVACY.md`
(редакция 1.3) на обоих языках. Обращение к константе живёт в одном месте — `InputAttributes`,
введённом E2b-3 — и E4c второго не заводит.

### Приватность (E4c)

Privacy-скан расширен на `latin/suggestions/` (`SuggestionsPackagePrivacyTest`): запрещены
`android.util.Log`, `println(`, `System.out`, `java.net.`, аналитика; отдельно проверено, что ни один
тип пакета не является `data class`, несущим строку. Именно здесь живёт детекция завершённого слова,
и до E4c у пакета не было privacy-гейта вообще.

### Гейт E4c на 2026-07-31

- Все сборки и `lintVitalRelease` — зелёные.
- JVM-набор: **563** → **590** тестов, 0 failures / 0 errors / 0 skipped.
- Release-APK: **1 506 103 Б**; дельта E4c — **+2 316 Б**.
- **Накопленная дельта фазы E4: +24 600 Б из бюджета 25 600 Б; остаётся 1 000 Б.**
- Абсолют: 1 506 103 Б ≤ hard limit 3 145 728 Б.
- `check-no-internet.sh` на СОБРАННОМ release-APK: единственное permission — `VIBRATE`,
  `allowBackup=false`, обе редакции правил без разрешающих элементов.
- Новых строк ресурсов, зависимостей и permission нет — очередь вычитки не пополняется.

### Изменён один существующий тест

`PersonalDictionaryContractTest.theOnlyWayToCreateAnEntryTodayIsTheExplicitManualAdd` пиннил
отсутствие обучения («E4c ещё не пришла»). E4c пришла, поэтому тест переписан в
`thereAreExactlyTwoWritePathsAndBothAreNamedInTheContract`: ручное добавление живёт в сторе и в
контроллере экрана, обучение — в сторе и в `PersonalLearning`, и ни одного третьего пути нет;
`SuggestionsController` по-прежнему не вызывает ни одной мутации.

### Устройство / замеры — статус для E4c

| device-пункт приёмки | статус | причина |
|---|---|---|
| «после двух завершений не предлагается, после трёх предлагается» вживую | NOT_COVERED | покрыто JVM (порог и пробеги), на устройстве не прогонялось |
| сессия из 50+ слов при выключенном тумблере не создаёт ни одного файла | NOT_COVERED | JVM-гейт есть, живой сессии не было |
| поля пароля / e-mail / URL / filter / NO_SUGGESTIONS / autocomplete / адрес — по одному прогону | NOT_COVERED | предикат покрыт source-contract тестами, живые поля не проверялись |
| direct boot: сессия до разблокировки не трогает файл | NOT_COVERED | JVM-гейт `lockedDeviceSessionNeverTouchesTheExistingFile`, устройства нет |
| дельта PSS фазы E4 и абсолютный PSS | NOT_COVERED | замер отложен решением владельца |
| latency обучения (ноль работы на UI-потоке) | NOT_COVERED | вся запись — события на фоновом executor, замера нет |

---

## E4d. «Забыть слово» долгим нажатием

Последняя подфаза E4 и первая в плане отхода — она уложилась в остаток бюджета только потому, что
все её строки были заведены ещё в E4b, и код обошёлся в 652 Б.

### Жест

Долгое нажатие по ячейке, показывающей слово из личного словаря, открывает «Забыть „X“?» поверх
окна IME (окно привязывается тем же `attachDialogToInputWindow`, что и пикер subtype, и гасится тем
же `hideWindow`). Подтверждение удаляет слово И его pending-хеш — иначе три следующих завершения
молча вернули бы только что удалённое слово. Долгое нажатие по словарному слову не делает ничего:
поиск просто не находит записи, и диалог не показывается.

**Принадлежность определяется бинарным поиском по параллельному массиву НОРМАЛИЗОВАННЫХ форм**
опубликованного снапшота, ключ — нормализованная форма ПОКАЗАННОЙ строки. Обе оговорки существенны:
снапшот хранит сырые формы, а показанная строка при INITIAL_CAPS и ALL_CAPS уже прошла `applyCasing`,
поэтому поиск по показанной строке при ALL-CAPS-префиксе не нашёл бы запись «Гүзәл» и жест молча не
делал бы ничего. Тест перебирает все три регистра, которые полоса может показать.

Поиск выполняется в теле отложенного `Runnable`, а не в `onTouchEvent`: требование «ноль аллокаций в
hot draw/touch» сохраняется, и это запинено тестом, который проверяет, что на пути касания нет ни
одного обращения к `suggestionAt(`. Таймер — одно заранее созданное поле-`Runnable` плюс
`postDelayed`/`removeCallbacks`. Срабатывание таймера отменяет tap этой последовательности касаний:
**долгое нажатие НИКОГДА не коммитит текст.**

### `ACTION_LONG_CLICK` — на всех заполненных ячейках, а не только на личных

Действие выставляется на КАЖДОЙ заполненной ячейке. Если бы оно стояло только на личных, состав
личного словаря стал бы наблюдаемым для любого включённого accessibility-сервиса и для
автоматических дампов дерева доступности: по наличию действия было бы видно, какое из трёх слов
пришло из приватного списка. Для словарного слова действие — no-op, чего контракт и так требует от
жеста, поэтому поведение не меняется, а утечка закрывается бесплатно.

### Гейт E4d на 2026-07-31

- Все сборки и `lintVitalRelease` — зелёные.
- JVM-набор: **590** → **598** тестов, 0 failures / 0 errors / 0 skipped.
- Release-APK: **1 506 755 Б**; дельта E4d — **+652 Б** (новых строк ресурсов подфаза не завела:
  `personal_dictionary_forget_title` и `personal_dictionary_delete` заведены в E4b).
- **Накопленная дельта фазы E4: +25 252 Б из бюджета 25 600 Б — уложились, остаток 348 Б.**
- Абсолют: 1 506 755 Б ≤ hard limit 3 145 728 Б; запас 1 638 973 Б.
- `check-no-internet.sh` на СОБРАННОМ release-APK: единственное permission — `VIBRATE`,
  `allowBackup=false`, обе редакции правил без разрешающих элементов.

### Устройство / замеры — статус для E4d

| device-пункт приёмки | статус | причина |
|---|---|---|
| долгое нажатие открывает диалог и НЕ вставляет текст | NOT_COVERED | таймер и отмена тапа покрыты source-contract, живого касания не было |
| долгое нажатие по словарному слову ничего не делает | NOT_COVERED | логика владения покрыта JVM, на устройстве не проверялась |
| TalkBack: долгое нажатие доступно на всех трёх ячейках | NOT_COVERED | действие выставляется в коде и запинено тестом, озвучка не проверялась |
| janky ≤ 1% и ноль аллокаций в hot touch при работающем таймере | NOT_COVERED | требует профайлера на устройстве |
| «забыл» переживает перезапуск клавиатуры | NOT_COVERED | запись покрыта JVM-тестами E4a-2, живого цикла не было |

---

## Итог фазы E4

Личный словарь работает целиком: формат и чтение (E4a-1), атомарная запись с инъекцией сбоя на
каждом шаге (E4a-2), тумблер, слияние в ранжирование и экран управления (E4b), обучение на трёх
чистых завершениях без хранения текста (E4c), «забыть слово» жестом (E4d).

Числа на выходе фазы: JVM-набор **483 → 598** тестов, release-APK **1 481 503 → 1 506 755 Б**
(дельта **+25 252 Б** из бюджета 25 600 Б), permission по-прежнему одно — `VIBRATE`.

Что остаётся незакрытым и почему — все таблицы «Устройство / замеры» выше; главное: ни один
device-пункт фазы не прогонялся, дельта PSS не измерена (замер отложен решением владельца), и 16
новых татарских строк ждут вычитки носителем языка.
