# PUBLISH-CHECKLIST — публикация v1.3.0

Пошаговый release gate для `versionName 1.3.0` / `versionCode 5`. Галочка означает
только проверенный результат именно кандидата v1.3.0. Исторические результаты v1.1.0 и
v1.2.0 не переносятся на новый APK и явно помечены как справочные.

## Кандидат перенумерован: 1.2.0/4 → 1.3.0/5 (оператор, 2026-08-18)

**Читать до начала выпуска.** Фазы E1–E5 и D3 (панель эмодзи, личный словарь, устойчивость
...
номере кандидата 1.2.0 и действует для v1.3.0 без изменений) — решение
...
## Artifact audit v1.3.0 выполнен (2026-08-19)
...
**Описывает предыдущий прогон, не кандидата этого чеклиста.** Тот же код, но артефакт
объявлял `versionCode 4` / `1.2.0`; перенумерован решением оператора — см. раздел «Кандидат
перенумерован» в начале файла. Оставлено как история.
...
см. раздел «Кандидат перенумерован» в начале файла и `docs/APK-AUDIT-1.3.0.md`.
...
артефакту 1.2.0; актуальное состояние кандидата v1.3.0 — в разделе «Artifact audit v1.3.0
выполнен» выше.
...
- [ ] Все intended изменения v1.3.0 находятся в одной проверяемой ветке; рабочее дерево
  не содержит случайных файлов, ключей или локальных конфигов. Частично: кандидат собран в
  ветке `codex/version-1.3.0` (отведена от `codex/apk-audit-2026-08-18`), бамп версии и
  changelog закоммичены (`1dc14afa`), `git diff --check` чист, ключи и `dist/` git-ignored.
  ...
  release commit не создан.
- [x] `app/build.gradle` подтверждает `versionName "1.3.0"` и `versionCode 5`. Проверено
  2026-08-19 в дереве `1dc14afa` (строки 15–16) и подтверждено на собранном артефакте.
- [x] В `CHANGELOG.md` есть раздел `[1.3.0]` (2026-08-19, фазы E1–E5 и D3), а
  `metadata/en-US/changelogs/5.txt` соответствует ему — сверено построчно 2026-08-19: шесть
  строк, по одной на раздел записи (эмодзи, личный словарь, опечатки, предсказание,
  автозамена, приватность), 407 Б при потолке Fastlane 500 Б. Историческая запись `[1.2.0]`
  и `changelogs/4.txt` не менялись. Финальная freeze-проверка повторяется перед release commit.
...
- [x] Полный JVM unit suite прошёл без failures/errors; записано число тестов — **709**
  (0 failures / 0 errors / 0 skipped), 73 suite-файла. Сборка 2026-08-19 из коммита
  `1dc14afa` (кандидат v1.3.0). То же число, что и 2026-08-18: бамп версии кода не касается.
  Историческое число прогона 2026-07-25 — 186 тестов на дереве `c3ed443`; рост объясняется
  фазами E1–E5 и D3, которые и стали содержимым версии 1.3.0.
- [x] `lintVitalRelease` завершился `BUILD SUCCESSFUL` (сборка 2026-08-19, коммит `1dc14afa`).
- [x] Debug и signed release APK собраны из того же frozen checkout: 2026-08-19 из коммита
  `1dc14afa` одной командой собраны `app-debug.apk` и подписанный `app-release.apk`
  (`BUILD SUCCESSFUL`, 84 actionable tasks: 84 executed).
- [x] Source, debug APK и release APK не содержат `android.permission.INTERNET`.
  Сборка 2026-08-19: `scripts/check-no-internet.sh` — exit 0 на обоих APK, оба уровня
...
- [x] Release APK подписан APK Signature Scheme v2 (v1=false, v2=true,
  ...
  → `Verifies`, exit 0. Проверено 2026-08-19 на `app-release.apk` и на копии
  `dist/tatar-keyboard-1.3.0.apk`.
- [ ] Signer certificate SHA-256 совпадает с историческим релизным сертификатом v1.1.0,
  ...
  2026-08-18 сгенерирован новый релизный ключ, сертификат артефакта v1.3.0 —
  ...
  пересоздавался — все три отпечатка совпали с прогоном 2026-08-18. Установка поверх сборки
  со старым ключом невозможна без удаления приложения. Пункт оставлен незакрытым намеренно,
  чтобы факт разрыва upgrade path не потерялся; см. «Подписная идентичность» в
  `docs/APK-AUDIT-1.3.0.md`.
- [x] `output-metadata.json` (`versionCode 5`, `versionName "1.3.0"`, `variantName
  release`) и manifest-разбор артефакта подтверждают версию. Сборка 2026-08-19:
  metadata release — `versionCode 5`, `versionName "1.3.0"`, `variantName release`,
  `applicationId org.tatarkeyboard.ime`; metadata debug — те же версия и `variantName debug`
  при `applicationId org.tatarkeyboard.ime.debug`; `aapt2 dump badging` — `versionCode='5'`,
  `versionName='1.3.0'`, `minSdkVersion:'24'`, `targetSdkVersion:'37'`.
- [x] Единственное запрошенное runtime/platform permission — `VIBRATE`; permission dump
  артефакта просмотрен целиком и содержит ровно одну строку. Сборка 2026-08-19:
...
- [x] APK не превышает 3 145 728 байт: **1 731 072** байта (сборка 2026-08-19,
  подписанный артефакт v1.3.0). Запас до потолка — 1 414 656 байт. Целевой бюджет D1
  ≤ 1,7 MiB — **pass** (1,6509 MiB). Размер совпал с артефактом 2026-08-18 до байта:
  изменились только `versionCode` и `versionName`, а `1.2.0` и `1.3.0` — строки одной
  длины. Историческое число 1 446 111 Б относится к утраченному артефакту от 2026-07-25.

### Evidence финального локального артефакта v1.3.0 — сборка 2026-08-19

Заполнять после последней пересборки; `PENDING` не заменять промежуточными числами.
Полный отчёт прогона — `docs/APK-AUDIT-1.3.0.md`.
...
**Пометка 2026-08-19: таблица описывает предыдущий прогон, не кандидата этого чеклиста.**
Тот же код, но артефакт объявлял `versionCode 4` / `1.2.0`; перенумерован решением
оператора — см. раздел «Кандидат перенумерован» в начале файла. Числа сохранены как история
и за свидетельство о v1.3.0 не выдаются.
...
**Пометка 2026-08-19: шаг 4 выполнен заново на кандидате v1.3.0.** Копия сделана из
подписанного `app-release.apk` сборки 2026-08-19 командным блоком ниже (`shasum -a 256`
заменён на `sha256sum` — Linux-эквивалент). Числа в трёх отметках ниже — свежие; всё, что
относится к прогону 2026-08-18 и к утраченному артефакту от 2026-07-25, помечено как
историческое. Подробности — `docs/APK-AUDIT-1.3.0.md`.

Рядом в `dist/` остаётся `tatar-keyboard-1.2.0.apk` от 2026-08-18 — артефакт прошлого
аудита, тот же код под старым номером версии. Он не удалён и **кандидатом на публикацию не
является**: кандидат — `dist/tatar-keyboard-1.3.0.apk`. Размер у файлов одинаковый, SHA-256
разные; различать по имени и по `aapt2 dump badging`.
...
cmp app/build/outputs/apk/release/app-release.apk dist/tatar-keyboard-1.3.0.apk
shasum -a 256 dist/tatar-keyboard-1.3.0.apk
apksigner verify --verbose --print-certs dist/tatar-keyboard-1.3.0.apk
```

- [x] `dist/tatar-keyboard-1.3.0.apk` совпадает с audited build output — `cmp` с
  `app/build/outputs/apk/release/app-release.apk` без различий, оба **1 731 072** байта,
  SHA-256 обоих `9f366d6cddfdb172fbd254a27afd91f238e51ac94bc34e5531040521cdaa74ea`,
  `apksigner verify` на копии → `Verifies` (v2 only, сертификат `98ca6feb…`). Прогон
  2026-08-19.
- [x] SHA-256 и все поля evidence выше обновлены после копирования (кроме frozen commit
  SHA — release commit ещё не создан; в таблице стоит HEAD ветки `1dc14afa`).
- [x] Повторная сборка после этого шага не выполнялась: mtime build output — 2026-08-19
  12:53:52, mtime копии в `dist/` — 12:54:12, копия позже сборки, содержимое побайтово
...
Версионные литералы в разделах 5–7 перецелены на v1.3.0 вместе со всем файлом (2026-08-19);
...
```sh
git tag -a v1.3.0 -m "Tatar Keyboard 1.3.0"
git push origin v1.3.0
```

- [ ] Tag `v1.3.0` существует на remote и указывает на audited frozen commit.
- [ ] GitHub Release имеет title `Tatar Keyboard 1.3.0`, release notes соответствуют
  `[1.3.0]` в CHANGELOG и содержит только `tatar-keyboard-1.3.0.apk`.
...
на публичный v1.3.0 Release. До подачи отдельно сверить актуальную
...
Его build, UAT или waiver не закрывают ни один checkbox v1.3.0.

**Пометка 2026-08-19: как эталон сертификата эта справка больше не действует.** Сертификат
...
(RSA 4096, `CN=Tatar Keyboard`), им подписаны артефакты 2026-08-18 и v1.3.0. Сверять подпись
кандидата со старым отпечатком нельзя: сравнение заведомо не пройдёт, upgrade path с v1.1.0
разорван намеренно. Подробности — «Подписная идентичность» в `docs/APK-AUDIT-1.3.0.md`.
