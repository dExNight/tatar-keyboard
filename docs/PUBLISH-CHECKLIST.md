# PUBLISH-CHECKLIST — публикация v1.5.0

Пошаговый release gate для `versionName 1.5.0` / `versionCode 7`. Галочка означает
только проверенный результат именно кандидата v1.5.0. Исторические результаты v1.1.0,
v1.2.0, v1.3.0 и v1.4.0 не переносятся на новый APK и явно помечены как справочные.

## Кандидат — v1.5.0 / versionCode 7 (оператор, 2026-08-20)

**Читать до начала выпуска.** Поверх проаудированного дерева v1.4.0 легла одна работа —
...
Актуальный кандидат — v1.5.0 / versionCode 7, см. раздел выше. Фазы E1–E5 и D3 (панель эмодзи, личный словарь, устойчивость
...
номере кандидата 1.2.0 и действует для v1.5.0 без изменений) — решение
...
## Artifact audit v1.5.0 выполнен (2026-08-20)
...
артефакту 1.2.0; актуальное состояние кандидата v1.5.0 — в разделе «Artifact audit v1.5.0
выполнен» выше.
...
- [ ] Все intended изменения v1.5.0 находятся в одной проверяемой ветке; рабочее дерево
  не содержит случайных файлов, ключей или локальных конфигов. Частично: работа по панели
  эмодзи и поиску закоммичена своей миссией (`bffcb73`, `736c838`, `f30589b`), ветка
  `codex/version-1.5.0` отведена от `codex/emoji-telegram`, статусы вычитки закоммичены
  (`c53d450`), бамп версии и changelog закоммичены (`4283944`), рабочее дерево на момент
  сборки чисто (`git status --short` пуст, `git diff --check` чист), ключи и `dist/`
  git-ignored. Открыто: служебный `.smgr/` не отслеживается; release commit не создан.
- [x] `app/build.gradle` подтверждает `versionName "1.5.0"` и `versionCode 7`. Проверено
  2026-08-20 в дереве `4283944` (строки 15–16) и подтверждено на собранном артефакте.
- [x] В `CHANGELOG.md` есть раздел `[1.5.0]` (2026-08-20, панель эмодзи по образцу
  Telegram и поиск по эмодзи), а `metadata/en-US/changelogs/7.txt` соответствует ему —
  сверено построчно 2026-08-20: шесть строк, по одной на смысловой пункт записи
  (перестроенная панель, крупные глифы и плавающие клавиши, поиск по русским и английским
  словам офлайн, результаты только из отрисовываемых эмодзи, запрос не попадает в поле
  ввода, приватность), 427 Б при потолке Fastlane 500 Б. Записи `[1.4.0]` и старше,
  `changelogs/6.txt` и старше не менялись. Финальная freeze-проверка повторяется перед
  release commit.
- [x] Новые татарские строки вычитаны носителем; имя проверяющего, дата и исправления
  записаны. **Правило изменено оператором 2026-08-20:** он снял требование своей отдельной
...
- [ ] В `docs/TATAR-REVIEW-QUEUE.tsv` нет ни одной строки в статусе `pending`, и у каждой
  строки заполнены `reviewer` и `date`. На 2026-08-20 из 45 строк 44 `approved` и одна
  остаётся `pending` — `E3a`, ссылка на `docs/DICTIONARY-E3-TYPO-REVIEW.tsv` (22 кандидата
...
- [ ] `git diff --check` и просмотр полного diff прошли перед release commit. Частично:
  `git diff --check` чист (прогон 2026-08-20 на `4283944`), полный diff перед release
...
- [x] Полный JVM unit suite прошёл без failures/errors; записано число тестов — **748**
  (0 failures / 0 errors / 0 skipped), 75 suite-файлов. Сборка 2026-08-20 из коммита
  `4283944` (кандидат v1.5.0). Было 723 на v1.4.0: **+25 тестов** дала переделка панели
  эмодзи и поиск — новый suite `EmojiSearchTest` (17), расширенный `EmojiPanelStateTest`
  (19 → 25) и `EmojiPackagePrivacyTest` (3 → 5). Историческое число прогона 2026-07-25 —
  186 тестов на дереве `c3ed443`.
- [x] `lintVitalRelease` завершился `BUILD SUCCESSFUL` (сборка 2026-08-20, коммит `4283944`).
- [x] Debug и signed release APK собраны из того же frozen checkout: 2026-08-20 из коммита
  `4283944` одной командой собраны `app-debug.apk` и подписанный `app-release.apk`
  (`BUILD SUCCESSFUL`, 84 actionable tasks: 84 executed).
- [x] Source, debug APK и release APK не содержат `android.permission.INTERNET`.
  Сборка 2026-08-20 (v1.5.0): `scripts/check-no-internet.sh` — exit 0 на обоих APK, оба уровня
...
- [ ] Signer certificate SHA-256 совпадает с историческим релизным сертификатом v1.1.0,
  ...
  2026-08-18 сгенерирован новый релизный ключ, сертификат артефакта v1.5.0 —
  ...
  пересоздавался — все три отпечатка совпали с прогонами 2026-08-18, 2026-08-19 и v1.4.0.
  Установка поверх сборки со старым ключом невозможна без удаления приложения. Пункт
  оставлен незакрытым намеренно, чтобы факт разрыва upgrade path не потерялся; см.
  «Подписная идентичность» в `docs/APK-AUDIT-1.5.0.md`.
- [x] `output-metadata.json` (`versionCode 7`, `versionName "1.5.0"`, `variantName
  release`) и manifest-разбор артефакта подтверждают версию. Сборка 2026-08-20:
  metadata release — `versionCode 7`, `versionName "1.5.0"`, `variantName release`,
  `applicationId org.tatarkeyboard.ime`; metadata debug — те же версия и `variantName debug`
  при `applicationId org.tatarkeyboard.ime.debug`; `aapt2 dump badging` — `versionCode='7'`,
  `versionName='1.5.0'`, `minSdkVersion:'24'`, `targetSdkVersion:'37'`.
- [x] Единственное запрошенное runtime/platform permission — `VIBRATE`; permission dump
  артефакта просмотрен целиком и содержит ровно одну строку. Сборка 2026-08-20 (v1.5.0):
...
- [x] APK не превышает 3 145 728 байт: **1 798 198** байт (сборка 2026-08-20,
  подписанный артефакт v1.5.0). Запас до потолка — 1 347 530 байт. **Целевой бюджет D1
  ≤ 1,7 MiB (1 782 579 Б) больше не выполняется:** 1,7147 MiB, перебор 15 619 Б. Прирост
  против v1.4.0 — **+65 170 Б**, и основная его часть — ассет индекса поиска
  `emoji_search_v1.txt`: 198 611 Б в исходнике, **54 766 Б сжатыми в APK** (84 % прироста);
  остальные ~10 КБ — код панели и поиска и новые строки. Обязательный потолок чеклиста
  3 145 728 Б — pass с запасом 43 %. Историческое число 1 446 111 Б относится к утраченному артефакту от
  2026-07-25.

### Evidence финального локального артефакта v1.5.0 — сборка 2026-08-20

Заполнять после последней пересборки; `PENDING` не заменять промежуточными числами.
Полный отчёт прогона — `docs/APK-AUDIT-1.5.0.md`.
...
**Пометка 2026-08-20: шаг 4 выполнен заново на кандидате v1.5.0.** Копия сделана из
подписанного `app-release.apk` сборки 2026-08-20 (v1.5.0) командным блоком ниже
(`shasum -a 256` заменён на `sha256sum` — Linux-эквивалент). Числа в трёх отметках ниже —
свежие; всё, что относится к прогонам v1.4.0, 2026-08-19 и 2026-08-18 и к утраченному
артефакту от 2026-07-25, помечено как историческое. Подробности —
`docs/APK-AUDIT-1.5.0.md`.
...
кандидат — `dist/tatar-keyboard-1.5.0.apk`. Различать по имени, по размеру (1 798 198 Б
против 1 733 028 Б у v1.4.0 и 1 731 072 Б у обоих более старых) и по
`aapt2 dump badging`.
...
cmp app/build/outputs/apk/release/app-release.apk dist/tatar-keyboard-1.5.0.apk
shasum -a 256 dist/tatar-keyboard-1.5.0.apk
apksigner verify --verbose --print-certs dist/tatar-keyboard-1.5.0.apk
```

- [x] `dist/tatar-keyboard-1.5.0.apk` совпадает с audited build output — `cmp` с
  `app/build/outputs/apk/release/app-release.apk` без различий, оба **1 798 198** байт,
  SHA-256 обоих `29902121835feb0a3bfa659f22557db8107732422d8e37766dfbd5ea56d1cb83`,
  `apksigner verify` на копии → `Verifies` (v2 only, сертификат `98ca6feb…`),
  `aapt2 dump badging` копии — `versionCode='7' versionName='1.5.0'`. Прогон 2026-08-20.
- [x] SHA-256 и все поля evidence выше обновлены после копирования (кроме frozen commit
  SHA — release commit ещё не создан; в таблице стоит HEAD ветки `4283944`).
- [x] Повторная сборка после этого шага не выполнялась: mtime build output — 2026-08-20
  04:22:36, mtime копии в `dist/` — 04:22:59, копия позже сборки, содержимое побайтово
...
Версионные литералы в разделах 5–7 перецелены на v1.5.0 вместе со всем файлом (2026-08-20);
...
- [ ] Tag `v1.5.0` существует на remote и указывает на audited frozen commit.
- [ ] GitHub Release имеет title `Tatar Keyboard 1.5.0`, release notes соответствуют
  `[1.5.0]` в CHANGELOG и содержит только `tatar-keyboard-1.5.0.apk`.
...
на публичный v1.5.0 Release. До подачи отдельно сверить актуальную
