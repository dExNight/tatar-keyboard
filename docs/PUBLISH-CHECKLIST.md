# PUBLISH-CHECKLIST — публикация v1.4.0

Пошаговый release gate для `versionName 1.4.0` / `versionCode 6`. Галочка означает
только проверенный результат именно кандидата v1.4.0. Исторические результаты v1.1.0,
v1.2.0 и v1.3.0 не переносятся на новый APK и явно помечены как справочные.

## Кандидат — v1.4.0 / versionCode 6 (оператор, 2026-08-20)

**Читать до начала выпуска.** Поверх проаудированного дерева v1.3.0 легли три
клавиатурные правки — переделка панели эмодзи (`docs/EMOJI-PANEL-REDESIGN.md`),
устойчивость клавиши `?123` при нажатии у края (`docs/SYMBOL-KEY-EDGE-FIX.md`) и
эргономика нижнего ряда: эмодзи с отдельной клавиши переехали на долгое нажатие запятой,
пробел вырос со 111 до 150dp (`docs/LAYOUT-ERGONOMICS.md`). Они сделали артефакт v1.3.0
недействительным как кандидата на публикацию, поэтому решением оператора от 2026-08-20
выходят **отдельной версией 1.4.0 / versionCode 6**.

Следствия для этого файла:

- Кандидат этого чеклиста — **v1.4.0 / versionCode 6**, коммит `660c34f`. Все `[x]`,
  проставленные ниже по прогону 2026-08-20, относятся к нему.
- Записи `[1.3.0]` и старше в `CHANGELOG.md`, а также `metadata/en-US/changelogs/5.txt`
  и старше не менялись. Новый набор описан разделом `[1.4.0]` и
  `metadata/en-US/changelogs/6.txt`.
- Числа прогонов 2026-07-25, 2026-08-18 и 2026-08-19 остаются в файле как история и
  помечены; за свидетельство о v1.4.0 они не выдаются.
- Полный отчёт прогона под новой версией — `docs/APK-AUDIT-1.4.0.md`.
...
Актуальный кандидат — v1.4.0 / versionCode 6, см. раздел выше. Фазы E1–E5 и D3 (панель эмодзи, личный словарь, устойчивость
...
номере кандидата 1.2.0 и действует для v1.4.0 без изменений) — решение
...
## Artifact audit v1.4.0 выполнен (2026-08-20)
...
артефакту 1.2.0; актуальное состояние кандидата v1.4.0 — в разделе «Artifact audit v1.4.0
выполнен» выше.
...
- [ ] Все intended изменения v1.4.0 находятся в одной проверяемой ветке; рабочее дерево
  не содержит случайных файлов, ключей или локальных конфигов. Частично: все три
  клавиатурные правки сведены в ветку `codex/version-1.4.0` (отведена от
  `codex/emoji-panel-redesign`), правка эргономики закоммичена (`84dba7c`), бамп версии и
  changelog закоммичены (`660c34f`), `git diff --check` чист, ключи и `dist/` git-ignored.
  Открыто: в рабочем дереве остаётся незакоммиченная правка `docs/TATAR-REVIEW-QUEUE.tsv`
  (проставляет `approved` строке E5d) — решение по ней за владельцем, на APK не влияет;
  служебный `.smgr/` не отслеживается; release commit не создан.
- [x] `app/build.gradle` подтверждает `versionName "1.4.0"` и `versionCode 6`. Проверено
  2026-08-20 в дереве `660c34f` (строки 15–16) и подтверждено на собранном артефакте.
- [x] В `CHANGELOG.md` есть раздел `[1.4.0]` (2026-08-20, три клавиатурные правки), а
  `metadata/en-US/changelogs/6.txt` соответствует ему — сверено построчно 2026-08-20: шесть
  строк, по одной на смысловой пункт записи (длинный пробел, эмодзи на долгом нажатии
  запятой, переделка панели, обрезанный последний ряд, край `?123`, приватность),
  392 Б при потолке Fastlane 500 Б. Записи `[1.3.0]` и старше, `changelogs/5.txt` и старше
  не менялись. Финальная freeze-проверка повторяется перед release commit.
...
- [x] Полный JVM unit suite прошёл без failures/errors; записано число тестов — **723**
  (0 failures / 0 errors / 0 skipped), 74 suite-файла. Сборка 2026-08-20 из коммита
  `660c34f` (кандидат v1.4.0). Было 709 на v1.3.0: +14 тестов дали три правки — панель
  эмодзи (`EmojiPanelStateTest`, 709 → 711), край `?123` (новый `SlidingModifierSlopTest`,
  711 → 719) и эргономика нижнего ряда (переписанный `SpaceKeyLayoutTest`, 719 → 723).
  Историческое число прогона 2026-07-25 — 186 тестов на дереве `c3ed443`.
- [x] `lintVitalRelease` завершился `BUILD SUCCESSFUL` (сборка 2026-08-20, коммит `660c34f`).
- [x] Debug и signed release APK собраны из того же frozen checkout: 2026-08-20 из коммита
  `660c34f` одной командой собраны `app-debug.apk` и подписанный `app-release.apk`
  (`BUILD SUCCESSFUL`, 84 actionable tasks: 84 executed).
- [x] Source, debug APK и release APK не содержат `android.permission.INTERNET`.
  Сборка 2026-08-20: `scripts/check-no-internet.sh` — exit 0 на обоих APK, оба уровня
...
- [ ] Signer certificate SHA-256 совпадает с историческим релизным сертификатом v1.1.0,
  ...
  2026-08-18 сгенерирован новый релизный ключ, сертификат артефакта v1.4.0 —
  ...
  пересоздавался — все три отпечатка совпали с прогонами 2026-08-18 и 2026-08-19. Установка
  поверх сборки со старым ключом невозможна без удаления приложения. Пункт оставлен
  незакрытым намеренно, чтобы факт разрыва upgrade path не потерялся; см. «Подписная
  идентичность» в `docs/APK-AUDIT-1.4.0.md`.
- [x] `output-metadata.json` (`versionCode 6`, `versionName "1.4.0"`, `variantName
  release`) и manifest-разбор артефакта подтверждают версию. Сборка 2026-08-20:
  metadata release — `versionCode 6`, `versionName "1.4.0"`, `variantName release`,
  `applicationId org.tatarkeyboard.ime`; metadata debug — те же версия и `variantName debug`
  при `applicationId org.tatarkeyboard.ime.debug`; `aapt2 dump badging` — `versionCode='6'`,
  `versionName='1.4.0'`, `minSdkVersion:'24'`, `targetSdkVersion:'37'`.
- [x] Единственное запрошенное runtime/platform permission — `VIBRATE`; permission dump
  артефакта просмотрен целиком и содержит ровно одну строку. Сборка 2026-08-20:
...
- [x] APK не превышает 3 145 728 байт: **1 733 028** байт (сборка 2026-08-20,
  подписанный артефакт v1.4.0). Запас до потолка — 1 412 700 байт. Целевой бюджет D1
  ≤ 1,7 MiB — **pass** (1,6528 MiB). Против артефакта v1.3.0 (1 731 072 Б) прирост
  **+1956 Б** — цена трёх клавиатурных правок (новые строки настроек, `<case>` в стилях
  клавиш и параметр `config_sliding_modifier_slop`). Историческое число 1 446 111 Б
  относится к утраченному артефакту от 2026-07-25.

### Evidence финального локального артефакта v1.4.0 — сборка 2026-08-20

Заполнять после последней пересборки; `PENDING` не заменять промежуточными числами.
Полный отчёт прогона — `docs/APK-AUDIT-1.4.0.md`.
...
свидетельство о v1.4.0 не выдаются. Полный отчёт того прогона — `docs/APK-AUDIT-1.3.0.md`.
...
**Пометка 2026-08-20: шаг 4 выполнен заново на кандидате v1.4.0.** Копия сделана из
подписанного `app-release.apk` сборки 2026-08-20 командным блоком ниже (`shasum -a 256`
заменён на `sha256sum` — Linux-эквивалент). Числа в трёх отметках ниже — свежие; всё, что
относится к прогонам 2026-08-19 и 2026-08-18 и к утраченному артефакту от 2026-07-25,
помечено как историческое. Подробности — `docs/APK-AUDIT-1.4.0.md`.

Рядом в `dist/` остаются `tatar-keyboard-1.2.0.apk` (2026-08-18) и
`tatar-keyboard-1.3.0.apk` (2026-08-19) — артефакты прошлых аудитов. Они не удалены и
**кандидатами на публикацию не являются**: кандидат — `dist/tatar-keyboard-1.4.0.apk`.
Различать по имени, по размеру (1 733 028 Б против 1 731 072 Б у обоих старых) и по
`aapt2 dump badging`.
...
cmp app/build/outputs/apk/release/app-release.apk dist/tatar-keyboard-1.4.0.apk
shasum -a 256 dist/tatar-keyboard-1.4.0.apk
apksigner verify --verbose --print-certs dist/tatar-keyboard-1.4.0.apk
```

- [x] `dist/tatar-keyboard-1.4.0.apk` совпадает с audited build output — `cmp` с
  `app/build/outputs/apk/release/app-release.apk` без различий, оба **1 733 028** байт,
  SHA-256 обоих `fea08bdd76e74c091e009ba73f18f6a3eac09a3574c9f256786717c5c218113c`,
  `apksigner verify` на копии → `Verifies` (v2 only, сертификат `98ca6feb…`),
  `aapt2 dump badging` копии — `versionCode='6' versionName='1.4.0'`. Прогон 2026-08-20.
- [x] SHA-256 и все поля evidence выше обновлены после копирования (кроме frozen commit
  SHA — release commit ещё не создан; в таблице стоит HEAD ветки `660c34f`).
- [x] Повторная сборка после этого шага не выполнялась: mtime build output — 2026-08-20
  02:49:20, mtime копии в `dist/` — 02:49:53, копия позже сборки, содержимое побайтово
...
Версионные литералы в разделах 5–7 перецелены на v1.4.0 вместе со всем файлом (2026-08-20);
...
```sh
git tag -a v1.4.0 -m "Tatar Keyboard 1.4.0"
git push origin v1.4.0
```

- [ ] Tag `v1.4.0` существует на remote и указывает на audited frozen commit.
- [ ] GitHub Release имеет title `Tatar Keyboard 1.4.0`, release notes соответствуют
  `[1.4.0]` в CHANGELOG и содержит только `tatar-keyboard-1.4.0.apk`.
...
на публичный v1.4.0 Release. До подачи отдельно сверить актуальную
...
Его build, UAT или waiver не закрывают ни один checkbox v1.4.0.

**Пометка 2026-08-19 (действует и для v1.4.0): как эталон сертификата эта справка больше
...
(RSA 4096, `CN=Tatar Keyboard`), им подписаны артефакты 2026-08-18, v1.3.0 и v1.4.0. Сверять
