#!/usr/bin/env python3
"""Hit-rate на РАЗГОВОРНОМ held-out — зеркало новостного замера (часть A, ru).

Новостный held-out `rus_news_2024_1M` — замороженный знаменатель E5a, но он меряет
жанр, который никто не печатает в мессенджере. Этот скрипт меряет то же правило
(событие — позиция, чей предыдущий токен прошёл normalize_word; попадание — цель в
видимой тройке) на разговорных строках, которые в обучении НЕ участвовали: прореживание
1/60 брал строки с id % 60 == 0, здесь берутся id % 60 == 1 — непересекающиеся выборки
одного дедуплицированного корпуса.

Запуск: measure_conv_heldout.py СТАРАЯ_ТАБЛИЦА НОВАЯ_ТАБЛИЦА ВЫХОД.json
"""
...
HELDOUT = ROOT / "build/corpus-conversational/rus_conv-heldout60-sentences.txt"
VISIBLE = 3
...
def main() -> int:
    ...
    alphabet = coverage.language_for("rus").alphabet
    old_table = load_table(old_path)
    new_table = load_table(new_path)
    retained = set(old_table) & set(new_table)
    report = {
        "heldout": str(HELDOUT),
        "note": "строки id % 60 == 1 дедуплицированного разговорного входа; в обучении (id % 60 == 0) не участвовали",
