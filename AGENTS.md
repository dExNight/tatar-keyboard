...
| Действие | Команда |
|---|---|
| JVM-тесты (1085 шт.) | `./gradlew test` (для честного прогона — `--rerun-tasks`) |
...
- Историю git и содержимое завершённых отчётов не переписываем; уточнения —
  ...
  Единичное исключение 2026-09-02 (решение оператора): история переписана
  `git filter-repo` для удаления трейлеров `Co-Authored-By: Claude` из тел
  226 коммитов (см. `docs/AUTHORSHIP-PLAN.md`, вариант B). Деревья не
  изменились; хэши коммитов в старых документах (до `dfcfb118`) ведут на
  прежнюю историю и не разрешаются — принято.
- **Авторство коммитов — только оператор.** Трейлеры `Co-Authored-By` от
  инструментов запрещены; сторож — `.githooks/commit-msg` (подключение:
  `git config core.hooksPath .githooks`); агентам не добавлять трейлеры
  и не выставлять себя автором/committer'ом.
...
- `app/src/test` — 1085 JVM-тестов (JUnit 4, Robolectric нет — осознанно).
