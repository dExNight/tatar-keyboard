...
> Практическое руководство для агентов (команды, гейты, ограничения, пины) — в корневом `AGENTS.md` (2026-08-31).
...
- **Performance** (жёсткие бюджеты): APK ≤ 3 МБ; PSS показанной клавиатуры — правило дельт на фазу, абсолютный потолок пересчитывается по первому валидному замеру на реальном устройстве (`docs/archive/PROPOSALS.md`, раздел «Бюджет памяти (PSS)»); холодный старт до показа < 400 мс (главная метрика); ноль аллокаций в цикле отрисовки; janky-кадры ~0%
