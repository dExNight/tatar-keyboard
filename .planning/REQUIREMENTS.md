...
- [ ] **LAYOUT-01**: Пользователь печатает на татарской раскладке: стандартная ЙЦУКЕН + отдельный видимый пятый ряд `ә ө ү җ ң һ`
...
- [ ] **LAYOUT-04**: Доступны слои цифр/символов `?123` и `#+=` с возвратом к буквам
- [ ] **LAYOUT-05**: Раскладки описаны данными (XML), не кодом; формат допускает добавление новой раскладки (латиница Zamanälif позже) без изменения движка
