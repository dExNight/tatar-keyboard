...
|---------|------|------|-------------|------------|-----------------|-----------|-------------------|-------------|--------|
| 05-01.T1 | 05-01 | 1 | INPUT-05 | 05-01/T1 | tryDoubleSpacePeriod (1100 мс, mIsPasswordField-гейт, isLetterOrDigit-чек), revert по backspace, always-on без pref; сборка зелёная | build + grep | verify-команда Task 1 (грепы по InputLogic + отсутствие getDoubleTapTimeout/pref_double_space) | ✅ (сборка фазы 1) | ⬜ pending |
| 05-01.T2 | 05-01 | 1 | INPUT-06 | — | Default true согласован ×3 (Settings.java + prefs XML + app_restrictions); pref_delete_swipe и механика не тронуты | build + grep + git test | verify-команда Task 2 (`PREF_SPACE_SWIPE, true` + два `defaultValue="true"` + diff-фильтр) | ✅ | ⬜ pending |
